from hygraph_core import HyGraph, HyGraphQuery, TimeSeries, TimeSeriesMetadata
import pandas as pd
...
node1 = hygraph.add_pgnode(
    oid=1, label="Node1", start_time=pd.to_datetime("2023-01-01")
)
node2 = hygraph.add_pgnode(
    oid=2, label="Node2", start_time=pd.to_datetime("2023-01-01")
)
...
metadata = TimeSeriesMetadata(
    owner_id=edge.getId(), element_type="edge"
)  # or whatever the correct metadata is
